...
- Global relations/rivalries pass is implemented in `planning/56_global_relations_rivalries_pass.md`: the empty vanilla relation/rivalry overrides now have conservative 1816 replacement layers. Relations cover the Congress great powers, colonial subjects, Spanish-American independence tension, USA/frontier tension, Ottoman/Persian/MENA tension, Qing tributary relationships, British India, Southeast Asia, Dutch East Indies, and a few African/Indian Ocean edges. Hard rivalries are intentionally sparse: Russia/Ottomans, Siam/Vietnam, Spain/Argentina, and Egypt/Nejd.
- Worldwide economy polish is implemented through the outlier/giant review in `planning/57_worldwide_economy_polish.md` and `planning/58_economy_outliers_and_giants_review.md`: no-design fixes folded stale Baltic `UBD` buildings into `RUS` and removed the inactive `SPC` Basque trade stub; the approved minimal construction floor added 30 one-level wooden construction sectors; the approved obvious port floor added basic ports to `PER`, `BER`, `SIA`, `SIN`, `MAD`, and `MAK` while skipping borderline inland `OYO`; and the approved outlier/giant package trimmed `BRZ`, `DEN`, `USA`, and `SWE`, left `SC3`/`NET`/`GBR`/`CHI` unchanged, added Hyderabad rural/resource texture, and gave `BIC` a small construction/Bombay trade-center bump. Remaining economy design cluster is later historical trade polish.
- Military polish follow-up is implemented in `planning/59_military_polish_review.md`: stale Baltic `UBD` formation units were folded into `RUS`, the explicitly marked ahistorical USA frigate blocks were removed, conservative starting formations were added for key playable countries that had barracks but no army, and tiny `COM`/`LKT` war-band formations were added for playability. Validation is clean with 0 brace problems, 0 banned late-unit tokens, and 0 formation owner-placement mismatches.
- Custom journal-entry Package A is implemented in `planning/60_custom_journal_entries_review.md`: obvious 1836/later start entries were removed or delayed for `USA`, `GBR`, `HAN`, `FRA`, `POR`, `BIC`, `CHI`, `HAI`, and `HUN`, along with directly related support flags/modifiers such as Missouri Compromise, Haiti indemnity payments, Orleanist France flags, post-Liberal-Wars Portugal flags, East Indies consolidation progress, and Qing warlord-explosion variables.
- Custom journal-entry Package B first wave is implemented from `planning/61_custom_journal_entry_menu.md`: all approved Option A and Option B entries were added as 84 custom `je_cow_1816_*` definitions, assigned through `common/history/global/99_cow_1816_custom_journal_entries.txt` to 101 playable tags and shared regional clusters, and localized in `localization/english/cow_1816_journal_entries_l_english.yml`. These are pinned historical-content GDP-growth objectives; custom event chains, rewards, and branching outcomes remain future design work.
- Custom journal-entry event/reward Package B1 is implemented in `planning/62_custom_journal_event_reward_design.md`: every custom 1816 JE now has an `on_complete` hook to one of 84 completion events in `events/cow_1816_journal_events.txt`, using 12 light five-year custom static modifiers in `common/static_modifiers/99_cow_1816_journal_modifiers.txt` and localization in `localization/english/cow_1816_journal_events_l_english.yml`. The implementation intentionally avoids follow-up chains, failures/timeouts, map changes, law changes, subject changes, war starts, and release effects until separately approved.
